...
class PollScheduler {
    ...
    async sendPollByType(pollType, title) {
        try {
            let chatId;
            let threadId;
    ...
    async sendPoll(chatId, pollType, title, threadId) {
        try {
            const today = new Date();
            const pollTitle = this.generatePollTitle(title, today);
            // Отправляем опрос
            const pollMessage = await this.bot.telegram.sendPoll(chatId, pollTitle, this.pollOptions, {
                is_anonymous: false,
            ...
            if (groupConfig) {
                await this.database.savePoll(groupConfig.id, pollMessage.message_id, pollType, today.toISOString().split('T')[0]);
            }
        }
        catch (error) {
    ...
    generatePollTitle(pollTitle, date) {
