...
import { Database, PollVote } from './database';

export class ReminderService {
  private bot: Telegraf<Context>;
  private database: Database;
  private reminderEndHour: number;
  ...
  private async processRemindersForGroup(groupType: 'city' | 'classic'): Promise<void> {
    try {
      let chatId: number;
      ...
      const votersToRemind = thinkingVoters.filter(voter => {
        if (!voter.lastReminderAt) return true;
  ...
  private async sendGroupReminder(chatId: number, threadId: number | undefined, voters: PollVote[], groupType: string): Promise<void> {
    try {
