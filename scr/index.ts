...
import { MessageCleaner } from './messageCleaner';
...
// Интерфейс для расширения контекста Telegraf
...
class CityPoolBot {
  ...
  constructor() {
    ...
    this.pollScheduler = new PollScheduler(this.bot as any, this.database);
    this.messageCleaner = new MessageCleaner(this.bot as any);
    this.reminderService = new ReminderService(this.bot as any, this.database);
  ...
  private setupCommands() {
    ...
    this.bot.command('renewpoll', async (ctx) => {
      try {
        ...
        let pollType: 'city' | 'classic' | null = null;
        let pollTitle = '';
        ...
        if (!pollType) {
          ...
          await ctx.reply(
            `ℹ️ Сегодня (${todayName}) нет игрового сбора.\n\n` +
            `${nextPollInfo}.\n\n` +
            `⏰ Ждите новый опрос завтра!`,
    ...
    this.bot.command('getid', async (ctx) => {
      try {
        ...
        const chatTitle = 'title' in (ctx.chat || {}) ? (ctx.chat as any).title : 'Личные сообщения';
  ...
  private setupMessageHandlers() {
    ...
    this.bot.on('poll_answer', async (ctx) => {
      try {
        ...
        if (!pollId || !optionIds || optionIds.length === 0 || !user) return;
